...
def test_model(feat_model_path, loss_model_path, part, add_loss, device):
    ...
    test_set = ASVspoof2019("PA", "/home/sarfaraz/ASVSpoof_2019_expts/CQCC_features/PA/pkl_features/",
                            "/home/sarfaraz/ASVSpoof_2019_expts/data/PA/ASVspoof2019_PA_cm_protocols/", part,
                            "LFCC", feat_len=750, padding="repeat")
    testDataLoader = DataLoader(test_set, batch_size=16, shuffle=False, num_workers=16,
                                collate_fn=test_set.collate_fn)
                                
    feat, _, _, _ = test_set[29]
    print("Feature shape", feat.shape)

    model.eval()
    
    tags_dict = {0: "-", 1: "AA", 2: "AB", 3: "AC", 4: "BA", 5: "BB", 6: "BC", 7: "CA", 8:"CB", 9:"CC"}
    ...
    with open(os.path.join(dir_path, 'checkpoint_cm_score.txt'), 'w') as cm_score_file:
        for i, (lfcc, audio_fn, tags, labels) in enumerate(tqdm(testDataLoader)):
            ...
            for j in range(labels.size(0)):
                #print(tags_dict[tags[j].data.cpu().numpy().tolist()])
                #cm_score_file.write(
                #    '%s A%02d %s %s\n' % (audio_fn[j], tags[j].data,
                #                          "spoof" if labels[j].data.cpu().numpy() else "bonafide",
                #                          score[j].item()))
                                        
                #print(tags[j].data.cpu().numpy())  
                cm_score_file.write(
                    '%s %s %s %s\n' % (audio_fn[j], tags_dict[tags[j].data.cpu().numpy().tolist()],
                                          "spoof" if labels[j].data.cpu().numpy() else "bonafide",
                                          score[j].item()))


    print(os.path.join(dir_path, 'checkpoint_cm_score.txt'))
    eer_cm, min_tDCF = compute_eer_and_tdcf(os.path.join(dir_path, 'checkpoint_cm_score.txt'),"/home/sarfaraz/ASVSpoof_2019_expts/data/")
    return eer_cm, min_tDCF

def test(model_dir, add_loss, device):
    model_path = os.path.join(model_dir, "anti-spoofing_lfcc_model.pt")
    loss_model_path = os.path.join(model_dir, "anti-spoofing_loss_model.pt")
    test_model(model_path, loss_model_path, "eval_manova10", add_loss, device)

def test_individual_attacks(cm_score_file):
    asv_score_file = '/home/sarfaraz/ASVSpoof_2019_expts/data/PA/ASVspoof2019_PA_asv_scores/ASVspoof2019.PA.asv.eval.gi.trl.scores.txt'
    ...
    for attack_idx in ['AA', 'AB', 'AC', 'BA', 'BB', 'BC', 'CA', 'CB', 'CC']:
        # Extract target, nontarget, and spoof scores from the ASV scores
        tar_asv = asv_scores[asv_keys == 'target']
        non_asv = asv_scores[asv_keys == 'nontarget']
        #spoof_asv = asv_scores[asv_sources == 'A%02d' % attack_idx]
        spoof_asv = asv_scores[asv_sources == attack_idx]
        
        # Extract bona fide (real human) and spoof scores from the CM scores
        bona_cm = cm_scores[cm_keys == 'bonafide']
        #spoof_cm = cm_scores[cm_sources == 'A%02d' % attack_idx]
        spoof_cm = cm_scores[cm_sources ==  attack_idx]

        # EERs of the standalone systems and fix ASV operating point to EER threshold
        eer_asv, asv_threshold = em.compute_eer(tar_asv, non_asv)
        eer_cm = em.compute_eer(bona_cm, spoof_cm)[0]
        eer_asv2, asv_threshold2 = em.compute_eer(tar_asv, spoof_asv)
        print(eer_asv2, eer_cm)

        #other_eer_cm = em.compute_eer(other_cm_scores[cm_keys == 'bonafide'], other_cm_scores[cm_sources == 'A%02d' % attack_idx])[0]
        other_eer_cm = em.compute_eer(other_cm_scores[cm_keys == 'bonafide'], other_cm_scores[cm_sources == attack_idx])[0]
        ...
        if eer_cm < other_eer_cm:
            ...
            tDCF_curve, CM_thresholds = em.compute_tDCF(other_cm_scores[cm_keys == 'bonafide'],
                                                        other_cm_scores[cm_sources == attack_idx],
                                                        Pfa_asv, Pmiss_asv, Pmiss_spoof_asv, cost_model, True)
            # Minimum t-DCF
            min_tDCF_index = np.argmin(tDCF_curve)
            min_tDCF = tDCF_curve[min_tDCF_index]
        eer_cm_lst.append(min(eer_cm*100, other_eer_cm*100))
