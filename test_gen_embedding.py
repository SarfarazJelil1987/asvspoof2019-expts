...
def test_model(feat_model_path, loss_model_path, part, add_loss, device):
    ...
    testDataLoader = DataLoader(test_set, batch_size=1, shuffle=False, num_workers=16,
    ...
    embeddings_store = np.zeros((134730,256))
    labels_store = np.zeros(134730)
    

    with open(os.path.join(dir_path, 'checkpoint_cm_score111.txt'), 'w') as cm_score_file:
        for i, (lfcc, audio_fn, tags, labels) in enumerate(tqdm(testDataLoader)):
            ...
            embeddings_store[i,:] = feats.data.cpu().numpy()
            labels_store[i] = 0 if labels.data.cpu().numpy() else 1
            

            #score = F.softmax(lfcc_outputs)[:, 0]

            
            #if len(labels_store==0) == 5000 and len(labels_store==1) == 50000:
            #    break
    ...
    print(embeddings_store.shape)
    np.save('tsne_data_CQCC/embeddings.npy',embeddings_store)
    np.save('tsne_data_CQCC/labels.npy', labels_store)
